...
function get_currencies_rates() {
    return wp_softgen_currency_rates()->get_currencies_rates();
}
...
function get_currencies() {
    return wp_softgen_currency_rates()->get_currencies();
}
 ...
 * @return array An associative array with currency codes for keys and currency data for values.
 */
function get_currency_rates_history($currency, $date) {
    return wp_softgen_currency_rates()->get_currency_rates_history($currency, $date);
}
...
function get_exchange_rates( $currency = 'USD' ) {
	...
	if ( is_array( $rates ) && $currency != 'USD' ) :
		...
		if ( ! currency_exists( $currency ) ) {
			trigger_error(
				__( 'WP Currencies: Base currency to get rates for was not found in database.', 'wp_currency_rates' ),
				E_USER_WARNING
			);
			return null;
		}
 ...
 *
 * @since 	1.0.0
 * @param  string    $from	  The currency code to convert from (default GEL).
 * @param  string    $to      The currency code to convert in (default USD).
 * @param  float|int $amount  The amount of currency to convert from (default 1 = exchange rate).
 *
 * @return float|int The resulting converted amount.
 */
function convert_currency( $from, $to, $amount = 0) {
    if ($from === $to){
        return $amount;
    } elseif ($from === 'GEL') {
        $rate = wp_softgen_currency_rates()->get_currency_rate($to);
        $currencies = wp_softgen_currency_rates()->currencies->get_currency($to);
        $correction = 1;
        if (array_key_exists('correction', $currencies)) {
            $correction = floatval($currencies['correction']);
        }
        return $amount / ($rate->sell_price * $correction);
    } elseif ($to === 'GEL') {
        $rate = wp_softgen_currency_rates()->get_currency_rate($from);
        $currencies = wp_softgen_currency_rates()->currencies->get_currency($from);
        $correction = 1;
        if (array_key_exists('correction', $currencies)) {
            $correction = floatval($currencies['correction']);
        }
        return $amount * $rate->buy_price * $correction;
    } else {
        return convert_currency( 'GEL', $to, convert_currency( $from, 'GEL', $amount));
    }

    /*
	$rates = wp_currency_rates()->get_rates();

	$error = $result = '';
	// check first if rates exist
	if ( $rates && is_array( $rates ) && count( $rates ) > 100 ) {

		if ( ! currency_exists( $from ) OR ! currency_exists( $to ) ) {
			trigger_error(
				__( 'WP Currencies: Currency does not exist or was not found in database.', 'wp_currency_rates' ),
				E_USER_WARNING
			);
			$error = true;
		}
		...
		if ( ! is_numeric( $amount ) ) {
			trigger_error(
				__( 'WP Currencies: Amount to convert must be a number.', 'wp_currency_rates' ),
				E_USER_WARNING
			);
			$error = true;
		}
		...
		trigger_error(
			__( 'WP Currencies: There was a problem fetching currency data from database. Is your API key valid?', 'wp_currency_rates' ),
			E_USER_WARNING
		);
...
function get_currency( $currency_code = 'USD' ) {
	...
	if ( ! is_string( $currency_code ) OR strlen( $currency_code ) != 3 ) {
		trigger_error(
			__( 'WP Currencies: you need to pass a valid currency code for argument and it must be a string of three characters long', 'wp_currency_rates' ),
			E_USER_WARNING
		);
		return null;
	}
	...
	if ( ! array_key_exists( strtoupper( $currency_code ), $currency_data ) ) {
		trigger_error(
			__( 'WP Currencies: the specified currency could not be found', 'wp_currency_rates' ),
			E_USER_WARNING
		);
		return null;
	}
...
function format_currency( $amount, $currency_code, $currency_symbol = true ) {
	...
	$currency = $currencies->get_currency( strtoupper( $currency_code ) );
	if ( is_null( $currency ) ){
		return '';
	}
	if ( ! $currency ) {
