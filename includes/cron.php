...
class Cron {
	...
	public function schedule_updates( $api_key = '', $interval = '' ) {
		...
		if ( empty( $api_key ) || empty(  $interval ) ) {
			$option = get_option( 'wp_softgen_currency_exchange_rates_settings' );
	...
	public function cron_schedules( $schedules ) {
		$schedules['weekly'] = array(
			'interval' => 604800,
			'display' => __( 'Once Weekly', 'wp_currency_rates' )
		);
		$schedules['biweekly'] = array(
			'interval' => 1209600,
			'display' => __( 'Once Biweekly', 'wp_currency_rates' )
		);
		$schedules['monthly'] = array(
			'interval' => 2419200,
			'display' => __( 'Once Monthly', 'wp_currency_rates' )
		);
		return $schedules;
	}
