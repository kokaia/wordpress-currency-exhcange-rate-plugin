...
class Settings {
	...
	public function add_action_links( $links ) {
		return array_merge(
			array( 'settings' => '<a href="' . admin_url( 'options-general.php?page=wp_currency_rates' ) . '">' . __( 'Currency Rates', 'wp_currency_rates' ) . '</a>' ),
			$links
		);
	}
	...
	public function add_admin_menu(  ) {
		add_options_page(
			__( 'WP Currency Rates', 'wp_currency_rates' ),
			__( 'Currency Rates', 'wp_currency_rates' ),
			'manage_options',
			'wp_currency_rates',
	...
	public function settings_init(  ) {
		...
		add_settings_section(
			...
			__( 'Settings', 'wp_currency_rates' ),
			array( $this, 'print_settings' ),
			'wp_currency_rates'
		);
		add_settings_field(
			'api_key',
			__( 'API Key', 'wp_currency_rates' ),
		...
		add_settings_field(
			'update_interval',
			__( 'Update Interval', 'wp_currency_rates' ),
...
/*
		...
		<label for="update_interval">
			<?php _e( 'Rates update frequency:', 'wp_currency_rates' ); ?>
			<select name="wp_currencies_settings[update_interval]" id="update_interval">
				<option value="hourly"   <?php selected( $update_frequency, 'hourly',   true ); ?>><?php _e( 'Hourly',  'wp_currency_rates' ); ?></option>
				<option value="daily"    <?php selected( $update_frequency, 'daily',    true ); ?>><?php _e( 'Daily',   'wp_currency_rates' ); ?></option>
				<option value="weekly"   <?php selected( $update_frequency, 'weekly',   true ); ?>><?php _e( 'Weekly',  'wp_currency_rates' ); ?></option>
				<option value="biweekly" <?php selected( $update_frequency, 'biweekly', true ); ?>><?php _e( 'Biweekly','wp_currency_rates' ); ?></option>
				<option value="monthly"  <?php selected( $update_frequency, 'monthly',  true ); ?>><?php _e( 'Monthly', 'wp_currency_rates' ); ?></option>
			</select>
		</label>
		<br>
		<small>
			<?php _e( 'Specify the frequency when to update currencies exchange rates', 'wp_currency_rates' ); ?>
		</small>
		<?php
*/
	}
	...
	public function print_api_key_field() {
		...
		?>
		<label for="api_key">
			<?php _e( 'Open Exchange Rates API key:', 'wp_currency_rates' ); ?>
			<input type="password" id="api_key" name="wp_currencies_settings[api_key]" value="<?php echo $api_key; ?>" class="regular-text">
		</label>
		<br>
		<small>
			<?php printf(
				_x( 'Get yours at: %1s', 'URL where to get the API key', 'wp_currency_rates' ),
	...
	public function print_settings(  ) {
		...
		<p>
			<?php printf(
				_x( 'WP Currencies pulls currency data from %1s and imports it into the WordPress database. 
				The exchange rates will be updated on a frequency that you can specify below.',
					'openexchangerates.org link', 'wp_currency_rates' ),
				'<a href="//openexchangerates.org" target="_blank">openexchangerates.org</a>' ); ?>
			<br />
			<?php _e( 'Please refer to plugin documentation for functions usage to help you creating with WordPress and WP Currencies.', 'wp_currency_rates' ); ?>
		</p>
		<?php
	}
	...
	public function options_page(  ) {
		...
		<form action="options.php" method="post">
			<h2><?php _e( 'WP Currencies', 'wp_currency_rates' ); ?></h2>
			<?php settings_fields( 'wp_currency_rates' ); ?>
			<?php do_settings_sections( 'wp_currency_rates' ); ?>
			<?php submit_button(); ?>
		</form>
		<?php
	}
